...
//detect hang mineral
    ...
    const double mineral_threshold_error = 13;  //水平方向error小于10认为对位完成
 ...
 * 3、二值化阈值说明：待补充
 */
    const double mineral_white_threshold_scale_low = 1.5;
    const double mineral_white_threshold_scale_high = 2.5;
    ...
    const double light_threshold_scale_low = 3;
    const double light_threshold_scale_high = 5;
    const double light_threshold_area_low = 200;
    const double light_threshold_area_high = 800;


    const int src_width_ = 1280;
    const int src_height_ = 720;
    const int error_max = 640;
//    const int offset = 46;
};